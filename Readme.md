Basic Alpine Edge based nginx from package
...
```console
$ docker run --name some-nginx -v /some/content:/usr/share/nginx/html:ro -d danielguerra/alpine-nginx
```

Alternatively, a simple `Dockerfile` can be used to generate a new image that includes the necessary content (which is a much cleaner solution than the bind mount above):

```dockerfile
FROM danielguerra/alpine-nginx
...
```console
$ docker run --name my-custom-nginx-container -v /host/path/nginx.conf:/etc/nginx/nginx.conf:ro -d danielguerra/alpine-nginx
```
...
```console
$ docker run --name tmp-nginx-container -d danielguerra/alpine-nginx
...
```yaml
web:
  image: danielguerra/alpine-nginx
  volumes:
